# -*- coding: utf-8 -*-
"""
Created on Mon Oct 21 23:35:32 2019
...
temp = []
for elem in AI_Assigned_Percentile:
    if (not elem):
        temp.append('0')
    else:
        temp.append(elem[0])
AI_Assigned_Percentile=temp
        
orig_df = orig_df.assign(AI_Assigned_Percentile=AI_Assigned_Percentile)
flagged_books = orig_df.copy(deep=True)
flagged_books = flagged_books[flagged_books['AI_Assigned_Percentile']=='bottom25p']
flagged_books = flagged_books[flagged_books['num_ratings'] <= np.percentile(orig_df['num_ratings'], 10)]
flagged_books = flagged_books[flagged_books['avg_num_stars'] <= np.percentile(orig_df['avg_num_stars'], 10)]
flagged_books = flagged_books[flagged_books['total_stars'] <= np.percentile(orig_df['total_stars'], 10)]
flagged_books = flagged_books[flagged_books['misspellings'] >= np.percentile(orig_df['misspellings'], 90)]
flagged_books = flagged_books[flagged_books['is_reviewed'] == False]

flagged_books.to_csv(r'C:\Users\Eli\Desktop\Python\THR\flagged_books.csv')

#flagged_books = flagged_books[(flagged_books['num_books_by_author'] >= np.percentile(orig_df['num_books_by_author'], 90)) or (flagged_books['num_books_by_author'] <= np.percentile(orig_df['num_books_by_author'], 10))]
#flagged_books = flagged_books[flagged_books['num_words'] >= np.percentile(orig_df['num_words'], 90) or flagged_books['num_words'] <= np.percentile(orig_df['num_words'], 10)]
